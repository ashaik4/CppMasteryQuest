...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // go in the list. To do this considering special
  ...
  Node * newNode = new Node(newData); //heap implementation
  ...
  if (empty() ) {
    pushFront(newNode->data); //push data to the front of list for empty list
    ...
    // check the first node and insert there if newNode < head
      ...
      while (!inserted) {
      	//insert the node before currNode if newNode->data <= currNode->data or if it's a nullptr (at end of list)
      	if (newNode->data <= currNode->data) {
      	  ...
      	  //update the next lin on the head side
